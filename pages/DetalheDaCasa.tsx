...
const DetalheDaCasa: React.FC = () => {
  ...
  const fetchDetails = async () => {
    ...
    if (casaRes.error) {
      ...
      const despesasList: Despesa[] = despesasRes.data || [];
      setDespesas(despesasList);

      // Busca nomes dos usuários que fizeram lançamentos
      const userIds = [...new Set(despesasList.map(d => d.user_id).filter(Boolean))] as string[];
      if (userIds.length > 0) {
  ...
  const handleGeneratePDF = async () => {
    ...
    if (isDateInvalid) { alert("A data final deve ser maior ou igual à data inicial."); return; }
    ...
    try {
      ...
      const auditPayload = {
        ...
        items: filteredDespesas.map(d => ({ d: d.data_lancamento, desc: d.descricao_original, v: d.valor }))
      };
      ...
      const tableData = filteredDespesas.map(d => [
        ...
        // Mostra o nome do usuário que lançou no PDF
        d.user_id ? (userNames[d.user_id] || 'Desconhecido') : '-'
      ]);

      autoTable(doc, {
        startY: 95,
        head: [['Data', 'Descrição', 'Qtd', 'Unit', 'Total', 'Categoria', 'Lançado por']],
        body: tableData,
        ...
        footStyles: { fillColor: [255, 255, 255], textColor: [249, 115, 22], fontStyle: 'bold', fontSize: 9, lineColor: [249, 115, 22], lineWidth: 0.1 },
        theme: 'striped',
        margin: { bottom: 30 }
      });
      ...
      const { error: uploadError } = await supabase.storage.from('extratos').upload(storagePath, pdfBlob, { contentType: 'application/pdf', upsert: true });
      if (uploadError) throw uploadError;

      const { data: signedData, error: signedError } = await supabase.storage.from('extratos').createSignedUrl(storagePath, 60 * 60 * 24);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      a.click();
      setTimeout(() => { window.URL.revokeObjectURL(objectUrl); document.body.removeChild(a); }, 100);
    } catch (err: any) {
  ...
  const handleEditClick = (d: Despesa) => { setExpenseToEdit(d); setMenuOpenId(null); };
  const handleAuditClick = (d: Despesa) => { setExpenseForAudit(d); setMenuOpenId(null); };
  ...
  // Dropdown de ações com controle de dono
  const ActionMenu = ({ d, isMobile = false }: { d: Despesa; isMobile?: boolean }) => {
    const isOwner = d.user_id === currentUserId;
    return (
      <div ref={menuRef} className={`absolute ${isMobile ? 'right-0' : 'right-0'} mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden animate-in fade-in zoom-in duration-150`}>
        {/* Editar */}
        {isOwner ? (
          <button onClick={() => handleEditClick(d)} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-gray-700 hover:bg-gray-50 transition-colors">
            <Pencil size={14} className="text-blue-500" /> Editar Registro
          </button>
        ) : (
          <div className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-gray-300 cursor-not-allowed" title="Você não pode editar lançamentos de outro usuário">
            <Pencil size={14} className="text-gray-200" /> Editar (não é seu)
          </div>
        )}

        {/* Histórico: todos podem ver */}
        <button onClick={() => handleAuditClick(d)} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-gray-700 hover:bg-gray-50 transition-colors border-b border-gray-50">
          <History size={14} className="text-orange-500" /> Ver Histórico
        </button>

        {/* Excluir */}
        {isOwner ? (
          <button onClick={() => { setExpenseToDelete(d); setMenuOpenId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-red-600 hover:bg-red-50 transition-colors">
            <Trash2 size={14} className="text-red-500" /> Excluir Item
          </button>
        ) : (
          <div className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-gray-300 cursor-not-allowed" title="Você não pode excluir lançamentos de outro usuário">
            <Trash2 size={14} className="text-gray-200" /> Excluir (não é seu)
          </div>
        )}
      </div>
    );
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-24 lg:pb-12">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/unidades')} className="p-2.5 hover:bg-white bg-gray-50 rounded-2xl transition-all shadow-sm border border-gray-100">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1">
              ...
              <button onClick={() => setShowStatusModal(true)} className={`flex items-center gap-1.5 px-2 py-0.5 border text-[9px] font-black uppercase tracking-wider rounded-lg transition-all ${getStatusBadgeClass(casa.status)}`}>
        ...
        <button onClick={() => setShowExpenseModal(true)} className="w-full lg:w-auto lg:self-end bg-orange-500 text-white px-6 py-4 rounded-2xl font-black shadow-lg shadow-orange-200 hover:bg-orange-600 active:scale-95 transition-all flex items-center justify-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
        <div className={`p-6 rounded-3xl border transition-all ${isOverBudget ? 'bg-red-50 border-red-200 shadow-sm' : 'bg-white border-gray-100 shadow-sm'}`}>
          ...
          <p className={`text-2xl font-black ${isOverBudget ? 'text-red-600' : 'text-gray-800'}`}>{formatCurrency(totalInvestido)}</p>
          <div className="mt-4 w-full bg-gray-100 h-2 rounded-full overflow-hidden">
            <div className={`h-full transition-all duration-700 ${isOverBudget ? 'bg-red-500' : 'bg-orange-500'}`} style={{ width: `${Math.min(percentualOrcamento, 100)}%` }} />
          </div>
          <p className="text-[10px] text-gray-400 font-bold mt-2 text-right">{percentualOrcamento.toFixed(1)}% do previsto</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <p className={`text-2xl font-black ${isOverBudget ? 'text-red-500' : 'text-green-600'}`}>{formatCurrency(orcamentoPrevisto - totalInvestido)}</p>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-gray-50 pb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-1">Filtre por período e gere um documento profissional com rastreabilidade de usuários.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5"><Calendar size={12} /> Data Inicial</label>
              <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-orange-500 outline-none font-bold text-sm transition-all shadow-sm" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5"><Calendar size={12} /> Data Final</label>
              <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className={`w-full px-4 py-3 bg-gray-50 border rounded-2xl focus:ring-2 focus:ring-orange-500 outline-none font-bold text-sm transition-all shadow-sm ${isDateInvalid ? 'border-red-300 ring-red-100 ring-2' : 'border-gray-200'}`} />
            </div>
            {isDateInvalid && <p className="col-span-1 sm:col-span-2 text-[10px] font-bold text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={12} /> Período inválido.</p>}
          </div>

          <div className="lg:col-span-3 bg-gray-50/50 rounded-2xl p-5 border border-dashed border-gray-200 shadow-inner">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-1.5"><Filter size={12} /> Filtro Ativo</p>
            <div className="space-y-3">
              <div className="flex justify-between items-center text-xs"><span className="text-gray-500 font-medium">Registros:</span><span className="font-black text-gray-800">{filteredDespesas.length}</span></div>
              <div className="flex justify-between items-center text-xs"><span className="text-gray-500 font-medium">Gasto no período:</span><span className="font-black text-orange-600">{formatCurrency(totalPeriodo)}</span></div>
            </div>
          </div>

          <div className="lg:col-span-4 flex flex-col gap-3">
            <button onClick={handleGeneratePDF} disabled={pdfLoading || filteredDespesas.length === 0 || !!isDateInvalid} className="w-full px-6 py-4 bg-gray-800 text-white rounded-2xl font-black text-sm hover:bg-gray-900 transition-all flex items-center justify-center gap-3 disabled:opacity-50 shadow-lg shadow-gray-200 active:scale-95">
              {pdfLoading ? <Loader2 size={18} className="animate-spin" /> : <FileDown size={18} />}
              Gerar Relatório Profissional
            </button>
            {signedPdfUrl && (
              <div className="flex gap-2 w-full">
                <button onClick={handleDownloadPDF} disabled={downloadLoading} className="flex-1 px-4 py-4 bg-orange-500 text-white rounded-2xl font-black text-sm hover:bg-orange-600 transition-all flex items-center justify-center gap-3 shadow-lg shadow-orange-100 disabled:opacity-70 active:scale-95">
                  {downloadLoading ? <Loader2 size={18} className="animate-spin" /> : <Download size={18} />} Baixar PDF
                </button>
                <button onClick={handleShareWhatsApp} className="p-4 bg-green-500 text-white rounded-2xl hover:bg-green-600 transition-all flex items-center justify-center shadow-lg shadow-green-100 active:scale-90" title="WhatsApp">
                  <Share2 size={24} />
                </button>
              </div>
            )}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b flex items-center justify-between bg-gray-50/30">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-orange-100 text-orange-600 rounded-xl"><Receipt size={20} /></div>
            <h3 className="font-black text-gray-800 text-sm md:text-base">Extrato Detalhado</h3>
          </div>
          <span className="text-[10px] md:text-xs bg-white border border-gray-100 px-3 py-1.5 rounded-full text-gray-600 font-bold uppercase tracking-wider">{despesas.length} Itens</span>
        </div>
              ...
              {despesas.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50/80 transition-all group">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[10px] text-gray-400 font-medium">{d.quantidade} un. x {formatCurrency(d.valor_unitario)}</p>
                    {d.observacao && <p className="text-[10px] text-gray-400 mt-1 italic max-w-xs truncate">{d.observacao}</p>}
                    {/* Quem lançou */}
                    {d.user_id && (
                      <p className="text-[9px] text-orange-500 font-black mt-1 flex items-center gap-1">
                        <UserCircle size={10} />
                        {d.user_id === currentUserId ? 'Você' : (userNames[d.user_id] || 'Outro usuário')}
                      </p>
                    )}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right relative">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {d.nota_fiscal_url && (
                        <a href={d.nota_fiscal_url} target="_blank" rel="noreferrer" className="p-2 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-xl transition-all" title="Nota Fiscal">
                          <ExternalLink size={18} />
                        </a>
                      )}
                      <div className="relative">
                        <button onClick={() => setMenuOpenId(menuOpenId === d.id ? null : d.id)} className={`p-2 rounded-xl transition-all ${menuOpenId === d.id ? 'bg-orange-100 text-orange-600' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600'}`}>
                          <MoreHorizontal size={18} />
                        </button>
                        {menuOpenId === d.id && <ActionMenu d={d} />}
                      </div>
                    </div>
                  </td>
                </tr>
          ...
          {despesas.map((d) => (
            <div key={d.id} className="p-4 space-y-3 active:bg-gray-50 transition-colors relative">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{new Date(d.data_lancamento).toLocaleDateString('pt-BR')}</p>
                  <p className="font-black text-gray-800 text-sm mt-0.5">{d.descricao_original}</p>
                  {d.user_id && (
                    <p className="text-[9px] text-orange-500 font-black mt-0.5 flex items-center gap-1">
                      <UserCircle size={10} />
                      {d.user_id === currentUserId ? 'Você' : (userNames[d.user_id] || 'Outro usuário')}
                    </p>
                  )}
                ...
                <div className="relative">
                  <button onClick={() => setMenuOpenId(menuOpenId === d.id ? null : d.id)} className={`p-2 rounded-xl border border-gray-100 shadow-sm ${menuOpenId === d.id ? 'bg-orange-500 text-white' : 'bg-white text-gray-400'}`}>
                    <MoreHorizontal size={18} />
                  </button>
                  {menuOpenId === d.id && <ActionMenu d={d} isMobile />}
                </div>
              </div>

              <div className="flex items-center gap-2">
                <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 bg-gray-100 text-gray-500 rounded-lg">{d.categorias_despesa?.nome}</span>
                {d.nota_fiscal_url && (
                  <a href={d.nota_fiscal_url} target="_blank" rel="noreferrer" className="p-1 text-orange-500 bg-orange-50 rounded-lg"><ExternalLink size={14} /></a>
                )}
              </div>

              <div className="flex justify-between items-end bg-gray-50/50 p-3 rounded-2xl border border-gray-50">
                <p className="text-[10px] text-gray-400 font-bold">{d.quantidade} x {formatCurrency(d.valor_unitario)}</p>
      ...
      {/* Modals */}
      {showExpenseModal && <ExpenseModal casaId={casa.id} onClose={() => setShowExpenseModal(false)} onSuccess={fetchDetails} />}
      {expenseToEdit && <ExpenseModal mode="edit" initialData={expenseToEdit} casaId={casa.id} onClose={() => setExpenseToEdit(null)} onSuccess={fetchDetails} />}
      {expenseForAudit && <AuditModal despesaId={expenseForAudit.id} descricaoOriginal={expenseForAudit.descricao_original} onClose={() => setExpenseForAudit(null)} />}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-end sm:items-center justify-center z-[100] p-0 sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in slide-in-from-bottom sm:zoom-in duration-300">
            <div className="p-6 border-b flex items-center justify-between">
              ...
              <button onClick={() => setShowStatusModal(false)} className="p-2 bg-gray-50 text-gray-400 hover:text-gray-600 rounded-xl"><X size={20} /></button>
            </div>
            <div className="p-4 sm:p-6 space-y-2">
              {(['Planejamento', 'Em Obra', 'Pausada', 'Finalizado', 'Vendido'] as Casa['status'][]).map((st) => (
                <button key={st} disabled={statusLoading} onClick={() => handleUpdateStatus(st)} className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl border-2 transition-all active:scale-95 ${casa.status === st ? 'bg-orange-50 border-orange-500 text-orange-700 font-black shadow-md' : 'bg-white border-gray-50 text-gray-600 hover:border-gray-200'}`}>
                  <div className="flex items-center gap-4">
                    <span className={`w-3.5 h-3.5 rounded-full ring-4 ring-white shadow-sm ${st === 'Planejamento' ? 'bg-gray-400' : st === 'Em Obra' ? 'bg-orange-500' : st === 'Pausada' ? 'bg-amber-500' : st === 'Finalizado' ? 'bg-green-500' : 'bg-blue-500'}`}></span>
            ...
            {statusLoading && <div className="p-5 bg-orange-500 flex items-center justify-center gap-3 text-white text-sm font-black animate-pulse"><Loader2 size={18} className="animate-spin" /> Atualizando unidade...</div>}
      ...
      {expenseToDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[110] p-0 sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-in slide-in-from-bottom sm:zoom-in duration-300">
            <div className="w-16 h-16 bg-red-100 text-red-600 rounded-3xl flex items-center justify-center mb-6 mx-auto shadow-sm"><Trash2 size={32} /></div>
            <h3 className="text-xl font-black text-gray-800 text-center mb-2">Excluir Registro?</h3>
            <p className="text-gray-500 text-sm text-center mb-8 px-4">Você está prestes a remover <span className="font-black text-gray-800">"{expenseToDelete.descricao_original}"</span> permanentemente.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <button disabled={isDeleting} onClick={() => setExpenseToDelete(null)} className="w-full py-4 border-2 border-gray-100 rounded-2xl text-gray-600 font-black text-sm hover:bg-gray-50 transition-all active:scale-95">Cancelar</button>
              <button disabled={isDeleting} onClick={handleDelete} className="w-full py-4 bg-red-500 text-white rounded-2xl font-black text-sm hover:bg-red-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-red-100 active:scale-95">
