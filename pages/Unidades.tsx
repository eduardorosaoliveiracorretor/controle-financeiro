...
const Unidades: React.FC = () => {
  ...
  const handleUpdateStatus = async (casa: Casa, novoStatus: Casa['status']) => {
    ...
    const { error } = await supabase.from('casas').update({ status: novoStatus }).eq('id', casa.id);
    if (error) {
  ...
  const handleDeleteCasa = async () => {
    if (!deletingCasa) return;
    setIsDeleting(true);
    // Deleta despesas da unidade primeiro
    await supabase.from('despesas').delete().eq('casa_id', deletingCasa.id);
    // Depois deleta a unidade
    const { error } = await supabase.from('casas').delete().eq('id', deletingCasa.id);
    if (error) {
      alert(`Erro ao excluir unidade: ${error.message}`);
    } else {
      setDeletingCasa(null);
      fetchData();
    }
    setIsDeleting(false);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {casas.map((casa) => (
            <div key={casa.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all group overflow-hidden">
              <div className="p-6 cursor-pointer" onClick={() => navigate(`/unidade/${casa.id}`)}>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); setStatusEditingCasa(casa); }}
                ...
                <div className="mt-4 space-y-2">
                  <p className="text-sm text-gray-500 flex items-center gap-2"><MapPin size={14} /> {casa.setores?.nome}</p>
                  <p className="text-sm text-gray-500 flex items-center gap-2"><Calendar size={14} /> Início: {new Date(casa.data_inicio_obra).toLocaleDateString()}</p>
                </div>
              </div>
              <div className="px-6 py-4 border-t bg-gray-50 flex items-center justify-between cursor-pointer" onClick={() => navigate(`/unidade/${casa.id}`)}>
                <div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-3">
              {(['Planejamento', 'Em Obra', 'Pausada', 'Finalizado', 'Vendido'] as Casa['status'][]).map((st) => (
                <button key={st} disabled={statusLoading} onClick={() => handleUpdateStatus(statusEditingCasa, st)}
                  className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl border transition-all hover:shadow-md ${statusEditingCasa.status === st ? 'bg-orange-50 border-orange-200 text-orange-700 font-bold' : 'bg-white border-gray-100 text-gray-600 hover:border-gray-300'}`}>
                  <div className="flex items-center gap-3">
                    <span className={`w-3 h-3 rounded-full ${st === 'Planejamento' ? 'bg-gray-400' : st === 'Em Obra' ? 'bg-orange-500' : st === 'Pausada' ? 'bg-amber-500' : st === 'Finalizado' ? 'bg-green-500' : 'bg-blue-500'}`}></span>
                    {st}
                  </div>
      ...
      {deletingCasa && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-in zoom-in duration-200">
            ...
            <h3 className="text-xl font-black text-gray-800 text-center mb-2">Excluir Unidade?</h3>
            <p className="text-gray-500 text-sm text-center mb-2 px-2">
              Você está prestes a remover <span className="font-black text-gray-800">"{deletingCasa.nome_casa}"</span>.
            </p>
            <p className="text-red-500 text-xs text-center font-bold mb-8">
              ⚠️ Todas as despesas vinculadas também serão excluídas permanentemente.
            </p>
            <div className="flex gap-3">
              <button disabled={isDeleting} onClick={() => setDeletingCasa(null)} className="flex-1 py-4 border-2 border-gray-100 rounded-2xl text-gray-600 font-black text-sm hover:bg-gray-50 transition-all">Cancelar</button>
              <button disabled={isDeleting} onClick={handleDeleteCasa} className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-black text-sm hover:bg-red-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-red-100">
            ...
            <form onSubmit={handleSubmit} className="p-4 md:p-8 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Nome da Unidade / Casa</label>
                  <input type="text" required value={formData.nome_casa} onChange={(e) => setFormData({...formData, nome_casa: e.target.value})} placeholder="Ex: Casa Jardim das Tulipas" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none transition-all text-sm" />
                </div>

                <div className="md:col-span-1">
                  <div className="flex items-center justify-between gap-1 mb-1.5">
                    <label className="text-sm font-bold text-gray-700">Setor / Localização</label>
                    <button type="button" onClick={() => setShowSectorModal(true)} className="text-xs text-orange-600 font-bold hover:underline">+ Novo Setor</button>
                  </div>
                  <select required value={formData.setor_id} onChange={(e) => setFormData({...formData, setor_id: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none text-sm">
                ...
                <div className="md:col-span-1">
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Orçamento Previsto (R$)</label>
                  <input type="number" required value={formData.orcamento_previsto} onChange={(e) => setFormData({...formData, orcamento_previsto: e.target.value})} placeholder="0,00" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none text-sm" />
                </div>

                <div className="md:col-span-1">
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Início da Obra</label>
                  <input type="date" required value={formData.data_inicio_obra} onChange={(e) => setFormData({...formData, data_inicio_obra: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none text-sm" />
                </div>

                <div className="md:col-span-1">
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">Status Inicial</label>
                  <select required value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value as any})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none text-sm">
      ...
      {showSectorModal && <SectorModal onClose={() => setShowSectorModal(false)} onSuccess={handleNewSectorSuccess} />}
    </div>
  );
};
